...
var=subprocess.check_output("echo \"file name;heuristique;strategie;cost;upper bound;rounded upper bound;gap;time\" > resultats/"+baseFileName+""+str(nb)+".csv",shell=True,errors=False)

i=0
for path in sorted(root.glob("**/*"),reverse=True):
    if(str(path)[len(str(path))-2:]=="kp" or str(path)[len(str(path))-2:]=="in" ):
        try:
            test=subprocess.check_output(buildPath+"/testBB ./"+str(path)+" resultats/"+baseFileName+""+str(nb)+".csv", shell=True,errors=False)
            #print("./"+str(path))
            #print(test)
            i+=1
        except:
        ...
        pass
    if(i==3): #NB max d'instances à tester
        break
