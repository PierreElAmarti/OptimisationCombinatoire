...
nb_nouv=(int(nb[:1])+1)
nb=int(nb[:1])
subprocess.check_output("touch resultats/res"+str(nb_nouv)+".csv",shell=True,errors=False)
subprocess.check_output("mv resultats/"+str(nb)+" resultats/"+str(nb_nouv)+"",shell=True,errors=False)
var=subprocess.check_output("echo \"file name;TYPE;cost;upper bound;rounded upper bound;gap;time\" > resultats/res"+str(nb_nouv)+".csv",shell=True,errors=False)

for path in root.glob("**/*"):
    if(str(path)[len(str(path))-2:]=="kp" or str(path)[len(str(path))-2:]=="in" ):
        try:
            test=subprocess.check_output(buildPath+"/Heur ./"+str(path)+" resultats/res"+str(nb_nouv)+".csv", shell=True,errors=False)
            test=subprocess.check_output(buildPath+"/BB ./"+str(path)+" resultats/res"+str(nb_nouv)+".csv", shell=True,errors=False)
            test=subprocess.check_output(buildPath+"/DP ./"+str(path)+" resultats/res"+str(nb_nouv)+".csv", shell=True,errors=False)
            #print("./"+str(path))
            i+=1
        except:
            pass
    ...
    print("fichier de sortie : resultats/res"+str(nb_nouv)+".csv")
